...
<div class="modal fade" id="uploadMediaModal" tabindex="-1" data-bs-backdrop="static"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="uploadMediaModalLabel">Add New Hero Media</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><form id="upload-form" novalidate><div class="mb-3"><label for="mediaName" class="form-label">Media Name</label><input type="text" class="form-control" id="mediaName" name="mediaName" required></div><div class="mb-3"><label for="uploaderName" class="form-label">Uploader</label><input type="text" class="form-control" id="uploaderName" name="uploaderName" required></div><div class="mb-3"><label for="mediaFile" class="form-label">Upload Photo or Video</label><input class="form-control" type="file" id="mediaFile" name="mediaFile" accept="image/*,video/*"><div class="form-text">Please select a landscape photo for best results.</div></div></form></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="button" class="btn btn-primary" id="save-media-btn">Save Media</button></div></div></div></div>
    ...
    <div class="modal fade" id="partner-modal" tabindex="-1" data-bs-backdrop="static">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="partner-modal-title">Add New Partner</h5>
                ...
                <div class="modal-body">
                    <form id="partner-form" novalidate>
                        ...
                        <div class="mb-3">
                            <label for="partner-link" class="form-label">Website Link</label>
                            <input type="url" class="form-control" id="partner-link" placeholder="https://example.com" required>
                        </div>
                        <div class="mb-3">
                            <label for="partner-logo" class="form-label">Partner Logo</label>
                            <input class="form-control" type="file" id="partner-logo" accept="image/*">
                            <div class="form-text">Upload a logo for the partner.</div>
                            <div class="text-center mt-2">
                                <img src="https://via.placeholder.com/100x100.png?text=Logo" alt="Logo Preview" id="partner-logo-preview" class="bg-light p-1" style="max-height: 80px;">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="partner-background" class="form-label">Background Image</label>
                            <input class="form-control" type="file" id="partner-background" accept="image/*">
                            <div class="form-text">This image appears behind the logo in the slider.</div>
                            <div class="text-center mt-2">
                                <img src="https://via.placeholder.com/200x100.png?text=Background" alt="Background Preview" id="partner-background-preview" class="bg-light p-1 img-fluid" style="max-height: 100px;">
                            </div>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="save-partner-btn">Add Partner</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <template id="exam-choicecard-template"><div class="p-3 border rounded mb-3 dynamic-exam-item"><div class="d-flex justify-content-between align-items-center mb-2"><h6 class="mb-0">Choice Card</h6><button type="button" class="btn-close remove-exam-item-btn"></button></div><div class="mb-2"><label class="form-label small">Card Image</label><input type="file" class="form-control form-control-sm choice-image" accept="image/*"></div><div class="mb-2"><label class="form-label small">Card Title</label><input type="text" class="form-control form-control-sm choice-title" required></div><div><label class="form-label small">Card Description</label><textarea class="form-control form-control-sm choice-description" rows="3" required></textarea></div></div></template>
    <!-- SCRIPTS -->
...
// Find and replace this entire function in your content_management.php file
...
(async function() {
    ...
    async function fetchAndRenderExams() {
        try {
            const response = await fetch(getApiPath('exams_handler.php?action=get'));
            const result = await response.json();
            if (result.status === 'success') {
                examsData = result.data;
                renderExamCards();
            } else { console.error("Failed to fetch exams:", result.message); }
    ...
    function renderExamCards() {
        ...
        examsData.forEach(exam => {
            ...
            const mediaUrl = exam.hero_media_path ? `../${exam.hero_media_path}` : 'https://via.placeholder.com/800x600.png?text=Exam+Media';
    ...
    function createDynamicElement(containerId, templateId, data = {}) {
        ...
        const itemEl = template.firstElementChild;
        container.appendChild(itemEl);
        
        if(data.image_path) {
            itemEl.dataset.existingImagePath = data.image_path;
        }
        ...
        if (templateId.includes('format')) {
            const selectEl = itemEl.querySelector('.format-icon');
            const previewEl = itemEl.querySelector('.icon-preview i');
            ...
            selectEl.value = data.icon_class || 'bi bi-info-circle';
            $(selectEl).select2({ dropdownParent: $('#exam-modal'), templateResult: formatIcon, templateSelection: formatIcon });
            $(selectEl).on('change', () => { previewEl.className = selectEl.value; });
            $(selectEl).trigger('change');
            itemEl.querySelector('.format-title').value = data.title || '';
            itemEl.querySelector('.format-description').value = data.description || '';
        } else if (templateId.includes('choicecard')) {
            itemEl.querySelector('.choice-title').value = data.title || '';
            itemEl.querySelector('.choice-description').value = data.description || '';
            if(data.image_path){
                const preview = document.createElement('div');
                preview.className = 'mt-2 border rounded p-2 text-center media-preview-container';
                preview.innerHTML = `<img src="../${data.image_path}" class="img-fluid rounded" style="max-height: 100px;"><p class="small text-muted mb-0 mt-1">${data.image_path.split('/').pop()}</p>`;
                itemEl.querySelector('.choice-image').after(preview);
            }
        } else if (templateId.includes('infocard')) {
            itemEl.querySelector('.infocard-title').value = data.title || '';
            itemEl.querySelector('.infocard-description').value = data.description || '';
        } else if (templateId.includes('faq')) {
            itemEl.querySelector('.faq-question').value = data.question || '';
            itemEl.querySelector('.faq-answer').value = data.answer || '';
        }

        itemEl.querySelector('.remove-exam-item-btn').addEventListener('click', () => {
            const selectEl = itemEl.querySelector('.format-icon');
            if (selectEl) $(selectEl).select2('destroy');
            itemEl.remove();
        });
    }
    ...
    function resetAndPrepareModal(mode = 'add', exam = null) {
        ...
        document.querySelectorAll('.media-preview-container').forEach(el => el.remove());
        ['exam-format-container', 'exam-choicecards-container', 'exam-infocards-container', 'exam-faq-container'].forEach(id => document.getElementById(id).innerHTML = '');
        new bootstrap.Tab(document.querySelector('#examTab button[data-bs-target="#exam-pane-info"]')).show();
        ...
        if (mode === 'edit' && exam) {
            ...
            document.getElementById('exam-about-content').value = exam.about_content;
            document.getElementById('exam-infocards-intro').value = exam.infocards_intro || '';

            ['hero_media', 'hero_logo', 'about_media'].forEach(type => {
                const path = exam[`${type}_path`];
                if(path) {
                    const input = document.getElementById(`exam-${type.replace('_', '-')}`);
                    if (input) {
                        const preview = document.createElement('div');
                        preview.className = 'mt-2 border rounded p-2 text-center media-preview-container';
                        preview.innerHTML = `<img src="../${path}" class="img-fluid rounded" style="max-height: 100px;"><p class="small text-muted mb-0 mt-1">${path.split('/').pop()}</p>`;
                        input.after(preview);
                    }
                }
            });
            ...
            (exam.choice_cards || []).forEach(d => createDynamicElement('exam-choicecards-container', 'exam-choicecard-template', d));
    ...
    examCardsContainer.addEventListener('click', (e) => { const card = e.target.closest('.service-card'); if (card) selectCard(card.dataset.id); });
    addExamBtn.addEventListener('click', () => { resetAndPrepareModal('add'); examModal.show(); });
    editExamBtn.addEventListener('click', () => { if (!selectedExamId) return; const exam = examsData.find(e => e.id == selectedExamId); resetAndPrepareModal('edit', exam); examModal.show(); });
    ...
    document.getElementById('add-exam-choicecard-btn').addEventListener('click', () => createDynamicElement('exam-choicecards-container', 'exam-choicecard-template'));
    ...
    deleteExamBtn.addEventListener('click', () => {
        ...
        confirmationModalTitle.textContent = "Confirm Deletion";
        confirmationModalBody.innerHTML = `Are you sure you want to delete the exam: <strong>${exam.name}</strong>? This cannot be undone.`;
        confirmActionBtn.className = 'btn btn-danger';
        confirmActionBtn.onclick = async () => {
            const formData = new FormData();
            formData.append('action', 'delete');
            formData.append('id', selectedExamId);
            const response = await fetch(getApiPath('exams_handler.php'), { method: 'POST', body: formData });
            const result = await response.json();
            if (result.status === 'success') {
                selectedExamId = null;
                fetchAndRenderExams();
    ...
    previewExamBtn.addEventListener('click', () => {
        if (!selectedExamId) return;
        const exam = examsData.find(e => e.id == selectedExamId);
        if (exam && exam.file_path) {
            window.open(`../${exam.file_path}`, '_blank');
        } else { alert('This exam does not have a page generated yet. Please save it first.'); }
    });
    ...
    saveExamBtn.addEventListener('click', async () => {
        ...
        formData.append('action', mode);
        if (mode === 'edit') formData.append('id', selectedExamId);
        
        formData.append('name', document.getElementById('exam-name').value);
        formData.append('description', document.getElementById('exam-description').value);
        formData.append('about_content', document.getElementById('exam-about-content').value);
        formData.append('infocards_intro', document.getElementById('exam-infocards-intro').value);
        
        if (document.getElementById('exam-hero-media').files[0]) formData.append('hero_media', document.getElementById('exam-hero-media').files[0]);
        if (document.getElementById('exam-hero-logo').files[0]) formData.append('hero_logo', document.getElementById('exam-hero-logo').files[0]);
        if (document.getElementById('exam-about-media').files[0]) formData.append('about_media', document.getElementById('exam-about-media').files[0]);
        
        if (mode === 'edit') {
            const exam = examsData.find(e => e.id == selectedExamId);
            if (!document.getElementById('exam-hero-media').files[0]) formData.append('existing_hero_path', exam.hero_media_path || '');
            if (!document.getElementById('exam-hero-logo').files[0]) formData.append('existing_hero_logo_path', exam.hero_logo_path || '');
            if (!document.getElementById('exam-about-media').files[0]) formData.append('existing_about_path', exam.about_media_path || '');
        }
        
        const formats = Array.from(document.querySelectorAll('#exam-format-container .dynamic-exam-item')).map(el => ({ icon: el.querySelector('.format-icon').value, title: el.querySelector('.format-title').value, description: el.querySelector('.format-description').value }));
        const choicecards = Array.from(document.querySelectorAll('#exam-choicecards-container .dynamic-exam-item')).map(el => ({ title: el.querySelector('.choice-title').value, description: el.querySelector('.choice-description').value }));
        const infocards = Array.from(document.querySelectorAll('#exam-infocards-container .dynamic-exam-item')).map(el => ({ title: el.querySelector('.infocard-title').value, description: el.querySelector('.infocard-description').value }));
        const faqs = Array.from(document.querySelectorAll('#exam-faq-container .dynamic-exam-item')).map(el => ({ question: el.querySelector('.faq-question').value, answer: el.querySelector('.faq-answer').value }));

        formData.append('formats', JSON.stringify(formats));
        formData.append('choicecards', JSON.stringify(choicecards));
        formData.append('infocards', JSON.stringify(infocards));
        formData.append('faqs', JSON.stringify(faqs));

        document.querySelectorAll('#exam-choicecards-container .dynamic-exam-item').forEach((item, index) => {
            const fileInput = item.querySelector('.choice-image');
            if (fileInput.files[0]) {
                formData.append(`choice_card_image_${index}`, fileInput.files[0]);
            } else if (mode === 'edit') {
                formData.append(`existing_choice_card_image_${index}`, item.dataset.existingImagePath || '');
            }
        });
...
(function() {
    ...
    const backgroundInput = document.getElementById('partner-background');
    const backgroundPreview = document.getElementById('partner-background-preview');
    
    const getApiPath = (file) => `../api/${file}`;

    async function fetchAndRenderPartners() {
        try {
            const response = await fetch(getApiPath('partners_handler.php?action=get'));
            const result = await response.json();
            if (result.status === 'success') {
    ...
    function renderTable() {
        ...
        partnersData.forEach(p => {
            ...
            const logoUrl = p.logo_path ? `../${p.logo_path}` : 'https://via.placeholder.com/30x30.png?text=L';
            row.innerHTML = `<td><img src="${logoUrl}" alt="${p.name}" class="me-2" style="width:30px; height:30px; object-fit:contain;">${p.name}</td><td><a href="${p.website_link}" target="_blank">${p.website_link}</a></td>`;
        });
    }

    function updateFabState() {
        const isSelected = selectedPartnerId !== null;
        editPartnerBtn.disabled = !isSelected;
        deletePartnerBtn.disabled = !isSelected;
        visitPartnerBtn.disabled = !isSelected;
    }

    function selectRow(partnerId) {
    ...
    function resetAndPrepareModal(mode = 'add', partner = null) {
        ...
        backgroundPreview.src = 'https://via.placeholder.com/200x100.png?text=Background';

        if (mode === 'add') {
            partnerModalTitle.textContent = 'Add New Partner';
            savePartnerBtn.textContent = 'Add Partner';
            savePartnerBtn.dataset.mode = 'add';
        } else if (mode === 'edit' && partner) {
            partnerModalTitle.textContent = `Edit Partner: ${partner.name}`;
            savePartnerBtn.textContent = 'Update Partner';
            savePartnerBtn.dataset.mode = 'edit';
            document.getElementById('partner-name').value = partner.name;
            document.getElementById('partner-link').value = partner.website_link;
            if (partner.logo_path) logoPreview.src = `../${partner.logo_path}`;
            if (partner.background_image_path) backgroundPreview.src = `../${partner.background_image_path}`;
        }
    }
    
    async function showPreviewOnChange(input, previewElement) {
        if (input.files && input.files[0]) {
            previewElement.src = await readFileAsDataURL(input.files[0]);
        }
    }
    ...
    logoInput.addEventListener('change', () => showPreviewOnChange(logoInput, logoPreview));
    backgroundInput.addEventListener('change', () => showPreviewOnChange(backgroundInput, backgroundPreview));
    ...
    editPartnerBtn.addEventListener('click', () => {
        ...
        resetAndPrepareModal('edit', partner);
        partnerModal.show();
    });
    
    visitPartnerBtn.addEventListener('click', () => {
        if (selectedPartnerId === null) return;
        const partner = partnersData.find(p => p.id == selectedPartnerId);
        window.open(partner.website_link, '_blank');
    });
    ...
    deletePartnerBtn.addEventListener('click', () => {
        ...
        const partner = partnersData.find(p => p.id == selectedPartnerId);
        confirmationModalTitle.textContent = "Confirm Deletion";
        confirmationModalBody.innerHTML = `Are you sure you want to delete the partner: <strong>${partner.name}</strong>? This cannot be undone.`;
        confirmActionBtn.className = 'btn btn-danger';
        confirmActionBtn.onclick = async () => {
            const formData = new FormData();
            formData.append('action', 'delete');
            formData.append('id', selectedPartnerId);
            const response = await fetch(getApiPath('partners_handler.php'), { method: 'POST', body: formData });
            const result = await response.json();
            if (result.status === 'success') {
    ...
    savePartnerBtn.addEventListener('click', async () => {
        ...
        formData.append('link', document.getElementById('partner-link').value);

        if (logoInput.files[0]) formData.append('logo_file', logoInput.files[0]);
        if (backgroundInput.files[0]) formData.append('background_file', backgroundInput.files[0]);
