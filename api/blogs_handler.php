...
switch ($action) {
    ...
    case 'add':
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO blogs (title, summary, author, publish_date, hero_media_path) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $blogTitle, $blogSummary, $blogAuthor, $blogDate, $heroPath);
            $stmt->execute();
    ...
    case 'edit':
        ...
        try {
            ...
            $stmt = $conn->prepare("UPDATE blogs SET title = ?, summary = ?, author = ?, publish_date = ?, hero_media_path = ? WHERE id = ?");
            $stmt->bind_param("sssssi", $blogTitle, $blogSummary, $blogAuthor, $blogDate, $heroPath, $blogId);
            $stmt->execute();
